...
@job_benefit_router.post('/job-benefits', status_code=201)
...
def _check_exist(benefit_id: int):
    ...
    r = http.request('GET', f'http://localhost:5004/benefits/{benefit_id}')
    if r.status != 200:
...
def detail_job_benefit(id: int, response: Response):
    with mydb:
        ...
        return True, job_benefit_result(job_benefit)
...
def all_job_benefit(page: int, limit: int, response: Response):
    with mydb:
        ...
        return job_benefit_list_result(job_benefit_list)


@job_benefit_router.put('/job-benefits/{id}', status_code=200)
async def update_job_benefit(id: int, req: JobBenefit, response: Response):
    ...
    ok, msg = __check_change(result, new_job_benefit)
    if ok is False:
        response.status_code = status.HTTP_400_BAD_REQUEST
        return msg
    # check job_id valid or not
...
def __check_change(old_result: dict, new_req: dict):
    old_result.pop('id')
    if new_req == old_result.copy():
        return False, "no information have been changed"
    return True, None


@job_benefit_router.delete('/job-benefits/{id}', status_code=200)
